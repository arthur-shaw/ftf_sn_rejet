...
food_dfs_renamed <- purrr::map(
    .x = food_dfs_raw,
    .f = ~ .x %>%
        # rename ID variable to common name
        dplyr::rename_with(
            .fn = ~ stringr::str_replace(., pattern = "sec8_1b_(cereales|viandes|poissons|huiles|laitier|fruits|legumes|legtub|sucreries|epices|boissons)__id", "aliment__id")
        ) %>%
        # harmonize variables starting with v
        dplyr::rename_with(
            .fn = ~ stringr::str_replace(., pattern = "(?<=v810[0-9]{1}[a-z]{0,1})_[0-9]+", "")
        ) %>%
        # harmonize variables starting with N
        dplyr::rename_with(
            .fn = ~ stringr::str_replace(., pattern = "(?<=N81000[abcde])_[0-9]+", "")
        )
)
